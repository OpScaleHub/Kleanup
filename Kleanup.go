...
on:
  ...
  # Trigger the FULL release process ONLY when a SemVer tag is pushed
  # Examples: v1.0.0, v1.2.3, v0.1.0-beta.1
  push:
    tags:
      - 'v[0-9]+.[0-9]+.[0-9]+'    # Matches vX.Y.Z
      - 'v[0-9]+.[0-9]+.[0-9]+-*' # Matches vX.Y.Z-prerelease.tag

  # Allow manual triggering (useful for testing the release process or forcing a release)
  workflow_dispatch:
